...
export default function AreaDetail() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // Fetch scores, checkins (30d for heatmap), and user settings in parallel
    const [scoresRes, checkinsRes, todayRes, userRes] = await Promise.all([
      supabase.from("score_daily").select("*").eq("area_id", id)
        .gte("date", startDate).order("date", { ascending: true }),
      supabase.from("checkins").select("date, completed").eq("area_id", id)
        .eq("user_id", user.id)
        .gte("date", format(subDays(new Date(), 29), "yyyy-MM-dd")),
      supabase.from("checkins").select("completed").eq("area_id", id)
        .eq("user_id", user.id).eq("date", today).single(),
      supabase.from("users").select("settings_score_visible").eq("user_id", user.id).single(),
    ]);

    if (scoresRes.data) {
      setScores(scoresRes.data.map((s) => ({ date: s.date, score: s.cumulative_score })));
    }

    const cMap: Record<string, boolean> = {};
    if (checkinsRes.data) {
      for (const c of checkinsRes.data) { cMap[c.date] = c.completed; }
    }
    setCheckinMap(cMap);
  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      // Refresh scores
      const startDate = format(subDays(new Date(), rangeToDays[timeRange]), "yyyy-MM-dd");
      const { data: newScores } = await supabase.from("score_daily").select("*")
        .eq("area_id", id).gte("date", startDate).order("date", { ascending: true });
      if (newScores) {
        setScores(newScores.map((s) => ({ date: s.date, score: s.cumulative_score })));
      }
    } catch {
  ...
  if (loading) {
    return (
      <motion.div
        ...
        className="flex flex-col px-4 pt-2 pb-8 gap-4"
      >
  ...
  return (
    ...
    >
      ...
      {hasData ? (
        ...
        <div className="rounded-xl bg-card flex flex-col items-center justify-center gap-4" style={{ height: "60vh" }}>
          ...
          <p className="text-sm text-muted-foreground text-center px-8">
            Keep observing. Your trajectory takes shape over days.
          </p>
        </div>
      )}
      ...
      {/* Calendar Heatmap */}
      <div className="mt-4">
        <CalendarHeatmap checkins={checkinMap} />
      </div>

      {/* Conditional score */}
      {scoreVisible && (
        <div className="mt-4 space-y-1">
          <p className="text-sm text-muted-foreground">Trajectory score</p>
      ...
      <div className="mt-4">
        <button
          ...
          className={`w-full min-h-[44px] rounded-lg text-base font-medium border transition-all flex items-center justify-center gap-2 ${
            todayCheckedIn
              ? "bg-[#7DA3A0]/20 text-[#7DA3A0] border-[#7DA3A0]"
              : "bg-transparent text-foreground border-[#7DA3A0]"
          } ${checkInLoading ? "opacity-50 cursor-not-allowed" : ""}`}
        >
          {checkInLoading ? (
            <div className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" />
          ) : todayCheckedIn ? "Observed" : "Log today"}
        </button>
      </div>

      {checkInError && (
        <p className="mt-2 text-sm text-destructive text-center">{checkInError}</p>
      )}

      {/* Edit area link */}
      <button
        onClick={() => navigate(`/areas/${id}/edit`)}
        className="mt-6 text-sm text-muted-foreground hover:text-foreground transition-colors min-h-[44px] self-center"
      >
        Edit area
      </button>
    </motion.div>
