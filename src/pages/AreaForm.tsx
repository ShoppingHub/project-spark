...
export default function AreaForm({ mode }: AreaFormProps) {
    ...
    try {
      if (mode === "add") {
        ...
        const { error: updateError } = await supabase
          .from("areas")
          .update({ name: trimmedName, type, frequency_per_week: frequency })
          .eq("id", id);
  ...
  return (
    <motion.div
      ...
      className="flex flex-col min-h-full px-4 pt-2 pb-8"
    >
      {/* Header */}
      <div className="flex items-center gap-3 h-14">
        <button
          onClick={() => navigate(mode === "edit" ? `/areas/${id}` : "/")}
          className="flex items-center justify-center h-10 w-10 min-h-[44px] min-w-[44px]"
        >
          <ArrowLeft size={24} strokeWidth={1.5} />
        </button>
        <h1 className="text-[18px] font-semibold">
          {mode === "add" ? "Add area" : "Edit area"}
        </h1>
      </div>
      ...
      <div className="flex flex-col gap-8 mt-4 flex-1">
        ...
        <div className="mt-auto pt-8 space-y-4">
          ...
          >
            ...
            {mode === "add" ? "Start observing" : "Save changes"}
          </button>
