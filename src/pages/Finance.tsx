...
const Finance = () => {
  ...
  const [financeArea, setFinanceArea] = useState<Area | null | undefined>(undefined); // undefined = loading
  ...
  const fetchData = useCallback(async () => {
    ...
    const [scoresRes, checkinsRes] = await Promise.all([
      supabase.from("score_daily").select("*").eq("area_id", area.id)
        .gte("date", startDate).order("date", { ascending: true }),
      supabase.from("checkins").select("id").eq("area_id", area.id)
  ...
  // Merge historic + projection into combined dataset
  const chartData = (() => {
    const historic = scores.map((s) => ({
      date: s.date,
      score: s.score,
    ...
    // Last historic point bridges to projection
    ...
    for (let i = 0; i <= 30; i++) {
      ...
      const projectedValue = lastScore + regression.m * i;
    ...
    // Connect: set projection on last historic point too
    if (historic.length > 0) {
      historic[historic.length - 1].projection = lastScore;
    }
    ...
    return (
      ...
      >
        <div className="h-14 flex items-center">
          <div className="h-5 w-20 rounded bg-card animate-pulse" />
        </div>
  ...
  if (financeArea === null) {
    return (
      <motion.div
        initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.3, ease: "easeInOut" }}
        className="flex flex-1 flex-col items-center justify-center gap-6 px-4 py-16"
      >
        <BarChart2 size={48} className="text-[#8C9496]" strokeWidth={1.5} />
        <p className="text-sm text-muted-foreground text-center">
          Log your first finance check-in to see your trend.
        </p>
        <button
          onClick={() => navigate("/areas/new?type=finance")}
    ...
    >
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 8, right: 8, bottom: 0, left: 0 }}>
            ...
            <Line type="monotone" dataKey="score" stroke={lineColor} strokeWidth={2}
              ...
              animationEasing="ease-in-out" connectNulls={false} />
            {/* Projection line */}
            {regression && (
              <Line type="monotone" dataKey="projection" stroke={lineColor}
                strokeWidth={2} strokeDasharray="4 4"
                dot={false} isAnimationActive animationDuration={300}
                animationEasing="ease-in-out" connectNulls />
            )}
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      {regression && (
        <p className="mt-4 text-sm text-muted-foreground text-center">
          30-day estimate based on your current trend.
        </p>
      )}
    </motion.div>
  );
